...
LinkedList *build_list(char *path, char *type, short attr)
{
	FILELIST *fl;
	LinkedList *head;
/*	FILELIST *last;*/
...
/* SMALLFILE STILL isn't changed to LinkedLists in this part yet*/
void consistency_check( LinkedList *first_set, LinkedList *progs, int progs_or)
{
	int found;
	/*FILELIST*/ LinkedList *flp;
	SMALLFILE *sfp;

	while(first_set!=NULL)
	{
		if(progs_or)
			sfp=((SET *)first_set->node)->progs;
		else
			sfp=((SET *)first_set->node)->accs;
		...
		while(sfp!=NULL)
		{
			...
			{
				if(!strcmp(sfp->name,((FILELIST *)flp->node)->name))
					found=1;
			...
			{
				if(sfp->left==NULL)
					((SET *)first_set->node)->progs=sfp->right;
				else
